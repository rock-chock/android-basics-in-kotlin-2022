...
class AddForageableFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Retrieve id from navigation args that was passed when clicking Edit button at ForageableDetailFragment
        val id = navigationArgs.id

        // If fragment has a passed id argument, then retrieve the item from the database
        // Otherwise set on click listener and add a new item
        if (id > 0) {

            // Observe a Forageable that is retrieved by id, set the forageable variable,
            // and call the bindForageable method to bind the xml variable to liveData variable
