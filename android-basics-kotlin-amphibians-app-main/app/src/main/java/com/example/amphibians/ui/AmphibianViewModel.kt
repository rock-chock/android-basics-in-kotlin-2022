...
class AmphibianViewModel : ViewModel() {
    ...
    // Call getAmphibiansList() on init so we can display status immediately
    init {
        getAmphibiansList()
    }
    ...
    fun getAmphibiansList() {
        // Launch the coroutine to ensure we're not blocking main thread
        viewModelScope.launch {
            // Set API status and use try-catch block for catching errors
            _status.value = AmphibianApiStatus.LOADING
            try {
                // Get list of amphibians via using Retrofit service's getAmphibians() method
