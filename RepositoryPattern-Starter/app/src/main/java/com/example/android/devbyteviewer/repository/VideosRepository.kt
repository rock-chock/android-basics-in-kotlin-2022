...
class VideosRepository(private val database: VideosDatabase) {
    ...
    suspend fun  refreshVideos() {
        withContext(Dispatchers.IO) {
            ...
            // Use playlist's asDatabaseModel() extension function  to map the playlist to the database object.
            database.videoDao.insertAll(playlist.asDatabaseModel())
        }
    }


    // LiveData object that holds a list of DevByteVideo objects.
    // getVideos() method returns a list of database objects, and not a list of DevByteVideo objects.
    // To fix the type mismatch, convert the list of database objects to a list of domain objects using the asDomainModel() conversion function.
